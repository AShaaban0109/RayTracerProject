...
//BEGIN_STAGE_ONE
    float kreflect;;
    Vector calculateReflection(Vector& viewer, Vector& normal);
    Vector calculateRefraction(Vector& viewer, Vector& normal, float ior);
	void fresnel(Vector& view, Vector& normal, float etai, float etat, float& kr);
	bool refract_ray(Vector& view, Hit& hit, float ior, Vector& refract_ray);
//END_STAGE_ONE
