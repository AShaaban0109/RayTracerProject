...
public:
	...
	AreaLight();
    AreaLight(Vertex centre, Vector up, float width, float height, Colour col);

	bool get_direction(Vertex &surface, Vector &dir);
	void get_intensity(Vertex &surface, Colour &intensity);
    void emitPhotons(int nGlobalPhotons, int nCausticPhotons, Photon* globalPhotons, Photon* causticPhotons);
